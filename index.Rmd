...
sort(round(props(all_surveys$occupational_status),2))
sort(round(props(all_surveys$occupational_status %contains% "student"),2))
sort(round(props(all_surveys$occupational_status %contains% "employed"),2))

codebook::plot_labelled(all_surveys$occupational_status) + coord_flip()
sort(round(props(haven::as_factor(all_surveys$relationship_status)),2))
codebook::plot_labelled(all_surveys$relationship_status) + coord_flip()
```
...
desire_items = s3_daily_id  %>% 
  filter(item_name %contains% "pair") %>% 
  filter(hidden == 0, !is.na(answer)) %>% 
  group_by(session, unit_session_id) %>% 
  mutate(display_order = min_rank(display_order),
         number_of_items_shown = n()) %>%  
...
```{r}
desire_items %>% 
  filter(item_name %contains% "in_pair") %>% 
  filter(day_number < 7) %>% 
  group_by(item_name) %>% 
  mutate(group_mean = mean(answer, na.rm = TRUE)) %>% 
  group_by(item_name, day_number, first_day_of_item_factor) %>% 
