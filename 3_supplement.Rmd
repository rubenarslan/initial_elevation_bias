...
first_page %>% filter(response_time_since_previous < 1*30*1000, response_time_since_previous > 0, display_order > 1) %>% 
  ...
  geom_line(position = position_dodge(width = 0.4), stat ='summary', fun.y = function(x) { mean(x, na.rm =T, trim = 0.1) }) +
  scale_y_continuous("Response time (in s) since previous item") +
  scale_x_continuous("No. of items shown", breaks = 1:7) +
  facet_wrap(~ label_english, scales = 'free_y')
```
...
```{r fig.cap="Figure 6 only showed the randomised variables. Here, we additionally show the estimates for the covariates.",fig.width=15,fig.height=11}
...
all_biases %>% show_dt() %>% 
  ...
  footnote("Estimates from the multilevel regression model displayed in Figure 6, with 95% confidence intervals.", threeparttable = TRUE) %>% 
  kable_styling(latex_options = "hold_position") %>% 
...
```{r fig.cap="Here, we fit the same model as before, but include only participants who participated the entire first week.",fig.width=15,fig.height=14}
predictors <- first_page %>% 
...
### Last item identity
```{r fig.cap="Here, we additionally test for an effect of last item identity (which is heavily collinear with item order).",fig.width=15,fig.height=18}
predictors <- first_page %>% 
  mutate(item_order = display_order,
         times_item_shown = times_item_answered,
         items_shown = number_of_items_shown
...
all_biases %>% 
  filter(is.na(group)) %>% 
  filter(term != "(Intercept)") %>% 
  filter(response != "relationship_satisfaction") %>% 
  left_join(
    first_page %>% select(response = item_name, item = label_english, item_mean, item_sd) %>% distinct()
  ) %>% 
...
```{r}
predictors %>% 
  filter(item_name != "relationship_satisfaction") %>% 
  split(.$label_english) %>%
  ...
  arrange(Correlation) %>% 
  knitr::kable(caption = "Residual correlation", format = "latex",  booktabs = TRUE) %>% 
  footnote("Correlation between the residuals of a covariate-only model and a model additionally including the randomised variables (including last item identity).", threeparttable = TRUE) %>% 
  kable_styling(latex_options = "hold_position") %>% 
  column_spec(1, width = "5cm") %>% 
  column_spec(2, width = "1.5cm")
```
...
```{r fig.cap="Here, we allow the effect of the randomised variables to vary by woman.",fig.width=15,fig.height=14}
predictors <- first_page %>% 
  mutate(item_order = display_order,
         times_item_shown = times_item_answered,
         items_shown = number_of_items_shown
...
```{r fig.cap="Here, we allow the effects of the randomised variables to be nonlinear.",fig.width=15,fig.height=18}
predictors <- first_page %>% 
...
time_items %>% group_by(item_name, label_english) %>% 
  ...
  knitr::kable(format = "latex", caption = "How often did how many women see each item?",  booktabs = TRUE) %>% 
  kable_styling(latex_options = "hold_position")
```
...
```{r fig.cap="Estimates for biases and covariates for the time items.",fig.width=15,fig.height=14}
...
all_biases %>% show_dt() %>% 
   knitr::kable(caption = "Multilevel regression coefficients", format = "latex", booktabs = TRUE, longtable = TRUE) %>%
  footnote("Estimates from the multilevel regression model, with 95% confidence intervals.", threeparttable = TRUE) %>% 
  kable_styling(latex_options = "hold_position") %>% 
  column_spec(1, width = "2.5cm") %>% 
...
```{r fig.cap="Estimates for biases and covariates for the desire items.",fig.width=15,fig.height=15}
