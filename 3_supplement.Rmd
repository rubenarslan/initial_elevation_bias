...
- frame other models as robustness checks
  - complete first week
  - don't adjust for confounders?
  - random effects?
  - nonlinear effects?
- make nice tables
- write up dropout section here and in ms
...
show_dt <- . %>% 
  ...
  mutate(estimate = if_else(is.na(conf.low), estimate, str_c(estimate, "\n[",conf.low, ";", conf.high, "]"))) %>% 
...
```{r fig.cap="As above, we can not only examine the mean but also the relative frequencies of  each response."}
first_page %>% 
...
```{r}
all_biases %>% show_dt() %>% 
   knitr::kable(format = "latex", booktabs = TRUE, caption = "Multilevel regression coefficients") %>%
...
time_items %>% group_by(item_name, label_english) %>% 
  ...
  knitr::kable(caption = "How often did how many women see each item?")
```
