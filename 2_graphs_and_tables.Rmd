...
all_surveys <- diary %>% ungroup() %>% select(session, didntmissfirstweek, hetero_relationship,
...
complex_mods <-  predictors %>% 
  split(.$item_name) %>%
  map(~ lmer(answer ~ weekday + items_shown + item_order + first_day_of_item_shown + poly(scale(day_number, scale=F)/10, degree = 3, raw = T) + refer_time_period +  (1 | session), data = .))
...
complex_mods_rt <-  predictors_rt %>% 
  split(.$item_name) %>%
  map(~ lmer(response_time_since_previous/1000 ~ weekday + items_shown + item_order + first_day_of_item_shown + poly(scale(day_number, scale=F)/10, degree = 3, raw = T) + refer_time_period +  (1 | session), data = .))
